...
void main()
{
    ...
    vec3 L = normalize(gl_LightSource[i].position.xyz - vec3(position); 
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        specularReflection += attenuation * vec3(gl_LightSource[i].specular) * vec3(gl_FrontMaterial.specular) * pow(max(dot(R,E),0.0),0.3*gl_FrontMaterial.shininess);
    }
